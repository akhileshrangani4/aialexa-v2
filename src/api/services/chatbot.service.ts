import { db, chatbots, chatbotFiles, conversations, messages } from '../../db';
...
export class ChatbotService {
  ...
  async getChatbot(chatbotId: string, userId?: string) {
    ...
    const chatbot = await db.select().from(chatbots)
      .where(conditions)
      .limit(1);
    ...
    const files = await db.select().from(chatbotFiles)
  ...
  async deleteChatbot(chatbotId: string, userId: string) {
    const deleted = await db.delete(chatbots)
      .where(and(
        eq(chatbots.id, chatbotId),
        eq(chatbots.user_id, userId)
      ))
