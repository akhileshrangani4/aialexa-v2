...
export function CreateChatbotModal({ isOpen, onClose }: CreateChatbotModalProps) {
  ...
  const onSubmit = (data: any) => {
    createMutation.mutate({
      ...
      suggestedQuestions: data.suggestedQuestions ? data.suggestedQuestions.split('\n').filter(Boolean) : [],
    });
  };
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                    />
                    {errors.systemPrompt && <p className="mt-1 text-sm text-red-600">{errors.systemPrompt.message as string}</p>}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Temperature (0-100)</label>
                      <input
                        type="number"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Suggested Questions (one per line)</label>
                    <textarea
